...
public class RadianMediaPlayerCtrl : MonoBehaviour {
	...
	public UnityEvent OnNearEndEvent ; 
	public UnityEvent OnFirstFrameEvent  ;
	...
	public void handHeldHelp(){
		Handheld.PlayFullScreenMovie(m_strFileName,Color.black);
	}

	void reLoad (string strName){
		StartCoroutine(reloadCouroutine(strName)) ; 
	}

	System.Collections.IEnumerator reloadCouroutine (string strName ){
		int loadTimes = 0 ; 
		bool success = false ; 
		do {
			yield return new WaitForSeconds(1f);
			success = Call_Load(strName,0) ;
			Debug.LogWarning("Radian: " + strName + " load failed... reloading at " + loadTimes + " times.");
			loadTimes++ ;
		}while (loadTimes < 10 && (!success)  ) ;
	}
	/// Radian
